package pl.jkiakumbo.player.ai;

import pl.jkiakumbo.BoardHelper;
...
public class Minimax {
    ...
    //print
    /*public static void printMM(int[][] board, int pl.jkiakumbo.player,int depth){
        BoardPrinter BP = new BoardPrinter(board,"Main");
        int bestScore = Integer.MIN_VALUE;
        Point bestMove = null;
        for(Point move : BoardHelper.getAllPossibleMoves(board,pl.jkiakumbo.player)){
            //create new node
            int[][] newNode = BoardHelper.getNewBoardAfterMove(board,move,pl.jkiakumbo.player);
            //recursive call
            int childScore = MMAB(newNode,pl.jkiakumbo.player,depth-1,false,Integer.MIN_VALUE,Integer.MAX_VALUE);
    ...
    /*private static int MM(int[][] node,int pl.jkiakumbo.player,int depth,boolean max){
        ...
        if(depth == 0 || BoardHelper.isGameFinished(node)){
            ...
            return Evaluator.eval(node,pl.jkiakumbo.player);
        }
        int oplayer = (pl.jkiakumbo.player==1) ? 2 : 1;
        //if no moves available then forfeit turn
        if((max && !BoardHelper.hasAnyMoves(node,pl.jkiakumbo.player)) || (!max && !BoardHelper.hasAnyMoves(node,oplayer))){
            System.out.println("Forfeit State Reached !");
            return MM(node,pl.jkiakumbo.player,depth-1,!max);
        }
        int score;
        if(max){
            //maximizing
            score = Integer.MIN_VALUE;
            for(Point move : BoardHelper.getAllPossibleMoves(node,pl.jkiakumbo.player)){ //my turn
                //create new node
                int[][] newNode = BoardHelper.getNewBoardAfterMove(node,move,pl.jkiakumbo.player);
                //recursive call
                int childScore = MM(newNode,pl.jkiakumbo.player,depth-1,false);
            ...
            for(Point move : BoardHelper.getAllPossibleMoves(node,oplayer)){ //opponent turn
                ...
                //recursive call
                int childScore = MM(newNode,pl.jkiakumbo.player,depth-1,true);
                if(childScore < score) score = childScore;
            }
        }
        return score;
